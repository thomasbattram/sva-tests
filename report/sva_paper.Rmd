...
fig_nums(name = "ncpg_plot")
ncpg_plot_cap <- fig_nums("ncpg_plot")
fig_nums(name = "covs_var_exp")
covs_var_exp_cap <- fig_nums("covs_var_exp")

# tables
table_nums(name = "nsv_summary")
nsv_summary_cap <- table_nums("nsv_summary")

# supplementary figures
sup_fig_nums(name = "smsva_sva_sv_plots")
smsva_sva_sv_plots_cap <- sup_fig_nums("smsva_sva_sv_plots")
sup_fig_nums(name = "model_param_time_plot")
model_param_time_plot_cap <- sup_fig_nums("model_param_time_plot")
sup_fig_nums(name = "time_plot2")
time_plot2_cap <- fig_nums("time_plot2")
sup_fig_nums(name = "est_num_plot")
...
## Results
The correlation between each of 20 SVs produced by the SmartSVA and SVA package was 1 or -1 when the parameters used were the same (__`r sup_table_nums("smsva_sva_sv_plots", display = "cite")`__) and the time taken by SmartSVA was faster when the sample size and number of CpGs were at the maximum, median = `r comma(median(sva_times / smsva_times))` times faster. When using the default alpha and B values in the SmartSVA package there was little change in time taken to using SmartSVA with the default parameters from the SVA package: __`r sup_fig_nums("model_param_time_plot", display = "cite")`__.
...
When subsetting the 450k array, the number of CpGs required to accurately estimate SVs was examined. When producing 20 SVs, over 90% of the variance of SV 1-`r print(sv_out_20k)` produced using the entire array was explained by all 20 SVs produced using a random subset of 20,000 CpGs. As the SV number increased, the variance explained by all SVs estimated from a subset of CpGs decreased (__`r fig_nums("ncpg_plot", display = "cite")`__). To explain over 90% of variance of all SVs, `r print(ncpg_out_20sv)` CpGs were required. Subsetting to the most variable CpGs produced SVs that explained less of the variance than when subsetting to random CpGs (__`r sup_fig_nums("time_plot2", display = "cite")`__).
...
The num.sv() function from the SVA package estimated fewer SVs for each of the X phenotypes tested, with a median of X fewer, compared to estimates using random matrix theory (__`r sup_fig_nums("est_num_plot", display = "cite")`__). 

The variance explained of various common EWAS covariates by the number of SVs estimated by random matrix theory varied substantially (__`r fig_nums("covs_var_exp", display = "cite")`__). As more SVs were added into the model, less additional variance was explained. For the 10 genomic PCs, all the SVs explained nearly none of the variance, but for the other covariates, a varying amount of variance was explained, which was fairly consistent across all the real and simulated traits assessed (__`r sup_fig_nums("", display = "cite")`__). <!-- The variance explained by 10, 20, 30 and the total number of SVs for each covariate for a simulated continuous trait and a real trait are shown in __`r table_nums("nsv_summary", display = "cite")`__. -->

```{r covs_var_exp, results = "asis"}
```
